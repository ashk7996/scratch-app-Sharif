...
struct SoundInputState {
    ...
    int activeVolInput = -1;
    int activePitchInput = -1;
} gSndInput;
...
void renderLogOverlay() {
    ...
    for (int i = start; i < (int) gLogEntries.size(); i++) {
        ...
        SDL_Color col = e.level == LOG_INFO
        ...
        renderText(line, px + 8, yOff, col, fontSmall);
        yOff += 24;
    }
}
...
string blockTypeToStr(BlockType t) {
    ...
    auto it = m.find(t);
    return it != m.end() ? it->second : "START";
}
...
BlockType strToBlockType(const string &s) {
    ...
    auto it = rev.find(s);
    return it != rev.end() ? it->second : START;
}
...
static string ep(const string &s) {
    string r;
    for (char c: s) {
        if (c == '|')r += "\\|";
        else r += c;
...
static Uint32 gErrorNotifyTick = 0;
static string gErrorNotifyMsg = "";

bool saveProject(const string &filename) {
    ofstream f(filename);
    if (!f) {
        cerr << "Cannot save: " << filename << "\n";
        return false;
    }
    // Serialization
    ...
    f << "SPIRIT|" << ep(gSpirit.spiritName) << "|" << gSpirit.spiritX << "|" << gSpirit.spiritY << "|"
            << gSpirit.direction << "|" << gSpirit.size << "|" << (gSpirit.isShow ? 1 : 0) << "|" << ep(
                gSpirit.texturePath) << "\n";
    for (int i = 0; i < 3; i++)
...
bool loadProject(const string &filename) {
    ifstream f(filename);
    if (!f) {
        cerr << "Cannot load: " << filename << "\n";
        return false;
    }
    string header;
    getline(f, header);
    if (header != "SCRATCH_V1") {
    ...
    while (getline(f, line)) {
        ...
        if (p[0] == "NAME" && p.size() >= 2) gCurrentProjectName = p[1];
            ...
            gSpirit.spiritName = p[1];
            gSpirit.spiritX = stoi(p[2]);
            gSpirit.spiritY = stoi(p[3]);
            gSpirit.direction = stoi(p[4]);
            gSpirit.size = stoi(p[5]);
            gSpirit.isShow = (p[6] == "1");
            ...
            b.blockMode = p[1];
            b.blockName = p[2];
            b.type = strToBlockType(p[3]);
            b.inputCount = stoi(p[4]);
            b.inputValue1 = p[5];
            b.inputValue2 = p[6];
            if (p.size() >= 12) {
                b.inputValue3 = p[7];
                b.bgColor = {(Uint8) stoi(p[8]), (Uint8) stoi(p[9]), (Uint8) stoi(p[10]), (Uint8) stoi(p[11])};
...
void newProject() {
    ...
    while (!gLoopStack.empty())gLoopStack.pop();
    gSpirit = Spirit();
    gCurrentProjectName = "Untitled";
    gTimerStartTick = SDL_GetTicks();
    loadSpiritTexture();
}
...
bool loadSpiritTexture() {
    ...
    gSpirit.texture = SDL_CreateTextureFromSurface(m_renderer, surf);
    SDL_FreeSurface(surf);
    return gSpirit.texture != nullptr;
}

void validatePosition() {
    gSpirit.spiritX = max(0, min(DW - 1, gSpirit.spiritX));
    gSpirit.spiritY = max(0, min(DH - 1, gSpirit.spiritY));
}

void validateDirection() {
    gSpirit.direction %= 360;
    if (gSpirit.direction < 0)gSpirit.direction += 360;
}
...
void setSpiritX(int x) {
    gSpirit.spiritX = x;
    validatePosition();
    logEvent(LOG_INFO, gExecuteIndex, "SET_X", "X=" + to_string(gSpirit.spiritX));
}

void setSpiritY(int y) {
    gSpirit.spiritY = y;
    validatePosition();
    logEvent(LOG_INFO, gExecuteIndex, "SET_Y", "Y=" + to_string(gSpirit.spiritY));
}

void moveNSteps(int n) {
    double rad = gSpirit.direction * M_PI / 180.0;
    gSpirit.spiritX += (int) (n * cos(rad));
    gSpirit.spiritY -= (int) (n * sin(rad));
    validatePosition();
    if (gSpirit.penDown)recordPenPoint();
...
void goRandom() {
    gSpirit.spiritX = rand() % DW;
    gSpirit.spiritY = rand() % DH;
    if (gSpirit.penDown)recordPenPoint();
}

void ifOnEdge() {
    bool edge = (gSpirit.spiritX == 0 || gSpirit.spiritX == DW - 1 || gSpirit.spiritY == 0 || gSpirit.spiritY == DH -
                 1);
    if (edge) {
        gSpirit.direction = (gSpirit.direction + 180) % 360;
        logEvent(LOG_WARNING, gExecuteIndex, "EDGE", "Bounced");
    }
}

void setSay(const string &txt, int ms = 0) {
    gSpirit.sayText = txt;
    gSpirit.sayUntil = ms > 0 ? SDL_GetTicks() + ms : 0;
}

void setThink(const string &txt, int ms = 0) {
    gSpirit.thinkText = txt;
    gSpirit.thinkUntil = ms > 0 ? SDL_GetTicks() + ms : 0;
}
...
void changeVar(const string &name, float delta) {
    for (auto &v: gVariables) {
        if (v.name == name) {
            float cur = 0;
            try { cur = stof(v.value); } catch (...) {
            }
            v.value = to_string(cur + delta);
            return;
        }
    }
}
...
string getVar(const string &name) {
    for (auto &v: gVariables)if (v.name == name)return v.value;
    return "";
}

//  SOUND PLAYBACK WITH PITCH SUPPORT
...
bool evalCondition(const string &expr) {
    ...
    for (auto &wop: vector<string>{"OR", "or"}) {
        ...
        if (p > 0) {
            return evalCondition(strim(e.substr(0, p))) || evalCondition(strim(e.substr(p + (int) wop.size())));
        }
    }
    ...
    for (auto &op: vector<string>{"==", "!=", "<=", ">="}) {
        ...
        if (p > 0) {
            float lv = toFloat(strim(e.substr(0, p)));
            float rv = toFloat(strim(e.substr(p + 2)));
            if (op == "==") return lv == rv;
            if (op == "!=") return lv != rv;
            if (op == "<=") return lv <= rv;
            if (op == ">=") return lv >= rv;
    ...
    for (auto &op: vector<string>{"<", ">"}) {
        ...
        if (p > 0) {
            float lv = toFloat(strim(e.substr(0, p)));
            float rv = toFloat(strim(e.substr(p + 1)));
            if (op == "<") return lv < rv;
            if (op == ">") return lv > rv;
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (t == CONTROL_IF || t == CONTROL_REPEAT || t == CONTROL_FOREVER || t == CONTROL_IF_ELSE)
            ...
            if (!cs.empty() && cs.top().second == CONTROL_IF_ELSE) {
                int o = cs.top().first;
                gExecuteList[o].jumpTarget2 = i + 1;
                gExecuteList[i].jumpTarget = o;
            }
...
void executeBlock(BlockType type, int idx) {
    ...
    switch (type) {
        case START: logEvent(LOG_INFO, idx, "START", "Execution started");
            break;
        case CONTROL_WAIT: {
            int ms = 1000;
            try { ms = (int) (stof(list[idx].inputValue1) * 1000); } catch (...) {
            }
            SDL_Delay(max(0, ms));
            break;
        }
        case CONTROL_REPEAT: {
            int count = 1;
            try { count = stoi(list[idx].inputValue1); } catch (...) {
            }
            if (count <= 0) { if (list[idx].jumpTarget >= 0)ei = list[idx].jumpTarget - 1; } else gLoopStack.
                    push(count);
            break;
        }
        case CONTROL_REPEAT_END: {
            if (!gLoopStack.empty()) {
                ...
                if (rem > 0) {
                    gLoopStack.top() = rem;
                    ei = list[idx].jumpTarget;
                } else gLoopStack.pop();
            }
            break;
        }
        case CONTROL_FOREVER: break;
        case CONTROL_FOREVER_END: ei = list[idx].jumpTarget;
            break;
        case CONTROL_IF: {
            bool ok = evalCondition(list[idx].inputValue1);
            if (!ok) {
                if (list[idx].jumpTarget >= 0)ei = list[idx].jumpTarget - 1;
                logEvent(LOG_INFO, idx, "IF", "FALSE");
        ...
        case CONTROL_IF_ELSE: {
            bool ok = evalCondition(list[idx].inputValue1);
            if (ok) { logEvent(LOG_INFO, idx, "IF_ELSE", "TRUE"); } else {
                if (list[idx].jumpTarget2 >= 0)ei = list[idx].jumpTarget2 - 1;
                logEvent(LOG_INFO, idx, "IF_ELSE", "FALSE");
            }
            break;
        }
        case CONTROL_ELSE: {
        ...
        case CONTROL_STOP_ALL: gIsExecuting = false;
            ei = 0;
            break;
        case MOTION_SETX: try { setSpiritX(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_SETY: try { setSpiritY(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_MOVENSTEP: try { moveNSteps(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_TURNNDEG: try { turnNDeg(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_TURNRIGHT: turnRight();
            break;
        case MOTION_TURNLEFT: turnLeft();
            break;
        case MOTION_CHANGEXBY: try { changeXBy(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_CHANGEYBY: try { changeYBy(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_CHANGEXYBY: try { goToXY(stoi(list[idx].inputValue1), stoi(list[idx].inputValue2)); } catch (...) {
            }
            break;
        case MOTION_RANDOMPOS: goRandom();
            break;
        case MOTION_CHANGEDEG: try { pointInDir(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case MOTION_IFONEDGE: ifOnEdge();
            break;
        case LOOKS_SAY: setSay(list[idx].inputValue1);
            break;
        case LOOKS_SAY_SECS: {
            int ms = 2000;
            try { ms = (int) (stof(list[idx].inputValue2) * 1000); } catch (...) {
            }
            setSay(list[idx].inputValue1, ms);
            break;
        }
        case LOOKS_THINK: setThink(list[idx].inputValue1);
            break;
        case LOOKS_SHOW: gSpirit.isShow = true;
            break;
        case LOOKS_HIDE: gSpirit.isShow = false;
            break;
        case LOOKS_SET_SIZE: try { setSize(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case LOOKS_CHANGE_SIZE: try { changeSize(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case LOOKS_CLEAR_EFFECTS: clearEffects();
            break;
        case PEN_DOWN: penDown();
            break;
        case PEN_UP: penUp();
            break;
        case PEN_ERASE_ALL: penErase();
            break;
        case PEN_STAMP: penStamp();
            break;
        case PEN_SET_SIZE: try { penSetSize(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case PEN_CHANGE_SIZE: try { penChgSize(stoi(list[idx].inputValue1)); } catch (...) {
            }
            break;
        case VAR_SET: setVar(list[idx].inputValue1, list[idx].inputValue2);
            break;
        case VAR_CHANGE: try { changeVar(list[idx].inputValue1, stof(list[idx].inputValue2)); } catch (...) {
            }
            break;
        case SOUND_PLAY_GAMEVOICE:
            playSound(0);
            logEvent(LOG_INFO, idx, "SND_GAMEVOICE",
                     "vol=" + to_string(gSoundSettings[0].volume) + " pitch=" + to_string(gSoundSettings[0].pitch));
            break;
        case SOUND_PLAY_SHEYPOR:
            playSound(1);
            logEvent(LOG_INFO, idx, "SND_SHEYPOR",
                     "vol=" + to_string(gSoundSettings[1].volume) + " pitch=" + to_string(gSoundSettings[1].pitch));
            break;
        case SOUND_PLAY_OPENING:
            playSound(2);
            logEvent(LOG_INFO, idx, "SND_OPENING",
                     "vol=" + to_string(gSoundSettings[2].volume) + " pitch=" + to_string(gSoundSettings[2].pitch));
            break;
        case SENSING_RESET_TIMER: gTimerStartTick = SDL_GetTicks();
            break;
        case SENSING_MOUSE_X: setVar("mouse_x", to_string(gMouse.x));
            break;
        case SENSING_MOUSE_Y: setVar("mouse_y", to_string(gMouse.y));
            break;
        ...
        case OP_ADD: {
            ...
            float a = toFloat(list[idx].inputValue1), b = toFloat(list[idx].inputValue2);
            setVar(rv, to_string(a + b));
            break;
        }
        case OP_SUBTRACT: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
        ...
        case OP_DIVIDE: {
            ...
            float a = toFloat(list[idx].inputValue1), b = toFloat(list[idx].inputValue2);
            if (b == 0) {
                logEvent(LOG_ERROR, idx, "DIVIDE", "Divided by 0 !");
                gErrorNotifyMsg = "Div by Zero!";
                gErrorNotifyTick = SDL_GetTicks();
                break;
            }
            setVar(rv, to_string(a / b));
            break;
        ...
        case OP_AND: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            bool a = evalCondition(list[idx].inputValue1), b = evalCondition(list[idx].inputValue2);
            setVar(rv, (a && b) ? "true" : "false");
            break;
        }
        case OP_OR: {
        ...
        case OP_NOT: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            bool a = evalCondition(list[idx].inputValue1);
            setVar(rv, (!a) ? "true" : "false");
            break;
        }
        // ────────────────────────── Math 1 input operators ──────────────────────────
        case OP_ABS: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            setVar(rv, to_string(fabs(toFloat(list[idx].inputValue1))));
            break;
        }
        case OP_SQRT: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            ...
            if (v < 0) {
                opError(idx, "Sqrt of negative numbers is not difined !" + to_string(v) + ")");
                break;
            }
            setVar(rv, to_string(sqrt(v)));
            break;
        }
        case OP_FLOOR: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            setVar(rv, to_string((int) floor(toFloat(list[idx].inputValue1))));
            break;
        }
        case OP_CEIL: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            setVar(rv, to_string((int) ceil(toFloat(list[idx].inputValue1))));
            break;
        }
        case OP_SIN: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            float deg = toFloat(list[idx].inputValue1);
            setVar(rv, to_string(sin(deg * M_PI / 180.0)));
            break;
        }
        case OP_COS: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            float deg = toFloat(list[idx].inputValue1);
            setVar(rv, to_string(cos(deg * M_PI / 180.0)));
            break;
        }
        // ────────────────────────── string operators ─────────────────────────────────────
        case OP_STR_LEN: {
            ...
            setVar(rv, to_string((int) s.size()));
            break;
        }
        case OP_STR_CHAR: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
            ...
            if (n < 1 || n > (int) s.size()) {
                opError(
                    idx, "CHAR_AT:  " + to_string(n) + "out of reach" + to_string(s.size()) + "");
                break;
            }
            setVar(rv, string(1, s[n - 1]));
            break;
        }
        case OP_STR_JOIN: {
            string rv = list[idx].inputValue3.empty() ? "op_result" : list[idx].inputValue3;
        ...
        case MOTION_GOTO_MOUSE: {
            ...
            goToXY(sx, sy);
            logEvent(LOG_INFO, idx, "GOTO_MOUSE", "→ (" + to_string(sx) + "," + to_string(sy) + ")");
            break;
        }
        default: logEvent(LOG_WARNING, idx, "UNKNOWN", "Block " + to_string((int) type));
            break;
    }
}
...
static void renderInputBox(int x, int y, int w, int h, const string &val, bool active) {
    ...
    if (!val.empty()) {
        int vw, vh;
        TTF_SizeUTF8(font, val.c_str(), &vw, &vh);
        renderText(val, x + (w - vw) / 2, y + (h - vh) / 2, black);
    }
}
...
void renderBlock(Block &block, int x, int y, bool isInMenu) {
    ...
    block.y = y;
    int tw, th;
    TTF_SizeUTF8(font, block.blockName.c_str(), &tw, &th);
    ...
    if (!isInMenu && gIsExecuting && gExecuteIndex < (int) gExecuteList.size()) {
        for (int i = 0; i < (int) gExecuteList.size(); i++) {
            if (&gExecuteList[i] == &block && i == gExecuteIndex) {
                ...
                SDL_Rect hl = {x + 25, y + 30, 240, 80};
                SDL_RenderFillRect(m_renderer, &hl);
                break;
            }
        }
    }
    ...
    if (!isInMenu) {
        ...
        if (inside && !inInput && gMouse.isClicked && !gIsDragging) {
            ...
            gDraggedBlock = block;
            for (int i = 0; i < (int) gExecuteList.size(); i++)
                if (&gExecuteList[i] == &block) {
                    gDraggedBlockIndex = i;
                    break;
                }
            gMouse.isClicked = false;
        }
...
void renderPenTrails() {
    for (int i = 1; i < (int) gPenTrail.size(); i++) {
        ...
        int hs = b.size / 2;
        for (int s = -hs; s <= hs; s++) {
            SDL_RenderDrawLine(m_renderer, ax + s, ay, bx + s, by);
            SDL_RenderDrawLine(m_renderer, ax, ay + s, bx, by + s);
        }
    }
}
...
void renderSpeechBubble() {
    ...
    if (gSpirit.sayUntil > 0 && now > gSpirit.sayUntil) {
        gSpirit.sayText = "";
        gSpirit.sayUntil = 0;
    }
    if (gSpirit.thinkUntil > 0 && now > gSpirit.thinkUntil) {
        gSpirit.thinkText = "";
        gSpirit.thinkUntil = 0;
    }
...
void renderBlockTypeIcon(const string &name, int x, int y, int r, SDL_Color color) {
    ...
    int tw = 0, th = 0;
    if (fontSmall)TTF_SizeUTF8(fontSmall, name.c_str(), &tw, &th);
...
void renderScrollableMenu(int x, int y, int w, int h) {
    ...
    for (auto &cat: getBlockLibrary()) {
        ...
        for (auto &block: cat.second) {
            renderBlock(const_cast<Block &>(block), x, cy, true);
            cy += 100;
        }
        break;
    }
...
void renderPlayground(int x, int y, int w, int h) {
    ...
    int cx = x + w / 2 - 120;
    for (int i = 0; i < (int) gExecuteList.size(); i++) {
        if (gIsDragging && i == gDraggedBlockIndex) {
            cy += 80;
            continue;
        }
    ...
    if (gIsDragging) {
        ...
        SDL_Rect ind = {x + 50, iy + 20, w - 100, 4};
        SDL_RenderFillRect(m_renderer, &ind);
    }
    SDL_RenderSetClipRect(m_renderer,NULL);
}
...
void renderCodePanel() {
    ...
    for (auto &cat: getBlockLibrary()) {
        ...
        if (isInRect({20, 205 + spacer, 100, 50}, gMouse.x, gMouse.y) && gMouse.isClicked) {
            gSelectedBlockType = cat.first;
            gMouse.scrolloffsetCodeMenu = 0;
        }
        spacer += 75;
    }
    ...
    if (!gVariables.empty()) {
        ...
        for (auto &v: gVariables) {
            ...
            renderText(v.name + " = " + v.value, vx + 5, vy + 5, {0, 0, 0, 255}, fontSmall);
            vy += 32;
        }
    }
}
...
void renderSoundsPanel() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        int ptw, pth;
        TTF_SizeUTF8(fontSmall, "Play", &ptw, &pth);
        renderText("Play", playBtn.x + (playBtn.w - ptw) / 2, playBtn.y + (playBtn.h - pth) / 2, {255, 255, 255, 255},
                   fontSmall);
        if (phov && gMouse.isClicked) {
            gMouse.isClicked = false;
            playSound(i);
        }
...
void renderModeSelector(vector<Button> &modes) {
    ...
    for (Button &btn: modes) {
        int origX = btn.rect.x;
        btn.rect.x = origX + spacer;
        if (string(btn.text) == gSelectedMode) {
            btn.currentColor = btn.focusColor;
            btn.currentTextColor = btn.focusTextColor;
        } else {
            bool hov = isInRect(btn.rect, gMouse.x, gMouse.y);
            btn.currentColor = hov ? btn.hoverColor : btn.normalColor;
...
void renderGamePreview() {
    ...
    if (gSpirit.isShow && gSpirit.texture) {
        ...
        SDL_FRect dst = {
            PREVIEW.x + (gSpirit.spiritX / (float) DW) * PREVIEW.w - fw / 2,
            PREVIEW.y + (gSpirit.spiritY / (float) DH) * PREVIEW.h - fh / 2, fw, fh
        };
        SDL_RenderCopyExF(m_renderer, gSpirit.texture, nullptr, &dst, -(gSpirit.direction - 90.0), nullptr,
                          SDL_FLIP_NONE);
    }
    renderSpeechBubble();
...
void renderSpiritStats() {
    ...
    Button showBtn = {
        {px + 30, py + 230, 220, 55}, gSpirit.isShow ? "Shown" : "Hidden",
        gTheme["bgSecondary"], gTheme["bgSecondary"], gTheme["primary"], gTheme["textSecondary"], gTheme["textPrimary"]
    };
    showBtn.currentColor = gSpirit.isShow ? gTheme["primary"] : gTheme["bgSecondary"];
    showBtn.currentTextColor = gTheme["textPrimary"];
    if (isInRect(showBtn.rect, gMouse.x, gMouse.y) && gMouse.isClicked) {
        gMouse.isClicked = false;
        gSpirit.isShow ^= true;
    }
    renderButton(showBtn);
    Button logBtn = {
        {px + 270, py + 230, 260, 55}, gShowLog ? "Log ON" : "Log OFF",
        gTheme["bgSecondary"], gTheme["bgSecondary"], {50, 50, 200, 255}, gTheme["textSecondary"], gTheme["textPrimary"]
    };
    logBtn.currentColor = gShowLog ? SDL_Color{50, 50, 200, 255} : gTheme["bgSecondary"];
    logBtn.currentTextColor = gTheme["textPrimary"];
    if (isInRect(logBtn.rect, gMouse.x, gMouse.y) && gMouse.isClicked) {
        gMouse.isClicked = false;
        gShowLog ^= true;
    }
    renderButton(logBtn);
    if (!gVariables.empty()) {
        renderText("Variables:", px + 30, py + 305, tc, fontSmall);
        int vy = py + 330;
        for (auto &v: gVariables) {
            renderText(v.name + " = " + v.value, px + 30, vy, tc, fontSmall);
            vy += 22;
            if (vy > py + ph - 20)break;
        }
    }
}
...
void renderMenu() {
    ...
    if (isInRect(fRect, gMouse.x, gMouse.y) && gMouse.isClicked) {
        gMouse.isClicked = false;
        gShowFileMenu ^= true;
    }
}
...
void renderFileDropdown() {
    ...
    for (int i = 0; i < 3; i++) {
        SDL_Rect r = {10, 71 + i * 45, 160, 44};
        bool hov = isInRect(r, gMouse.x, gMouse.y);
...
void renderRunButtons() {
    ...
    if (isInRect({PREVIEW.x, 125, 50, 50}, gMouse.x, gMouse.y) && gMouse.isClicked) {
        ...
        if (!gIsExecuting) {
            ...
            while (!gLoopStack.empty())gLoopStack.pop();
            preProcessBlocks();
        } else if (gIsPaused)gIsPaused = false;
        else gIsPaused = true;
    }
    ...
    Button dbg = {
        {DW - 220, 115, 200, 60}, gIsInDebug ? "Debug ON" : "Rush",
        gTheme["bgSecondary"], gTheme["bgWhite"], {160, 60, 255, 255}, gTheme["textSecondary"], gTheme["textPrimary"]
    };
    dbg.currentColor = gIsInDebug ? SDL_Color{160, 60, 255, 255} : gTheme["bgSecondary"];
    dbg.currentTextColor = gTheme["textPrimary"];
    if (isInRect(dbg.rect, gMouse.x, gMouse.y) && gMouse.isClicked) {
        gMouse.isClicked = false;
        gIsInDebug ^= true;
    }
    renderButton(dbg);
    if (gIsExecuting)
        renderText(gIsPaused
                       ? "PAUSED @ block " + to_string(gExecuteIndex)
                       : "Running block " + to_string(gExecuteIndex),
    ...
    if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
        ...
        if (gSpirit.isShow && gSpirit.texture && isInRect(PREVIEW, mx, my) && !gIsExecuting) {
            ...
            if (isInRect(sprRect, mx, my)) {
                ...
                while (!gLoopStack.empty())gLoopStack.pop();
                preProcessBlocks();
                logEvent(LOG_INFO, 0, "SPRITE_CLICK", "runed when spirit clicked");
                gMouse.isClicked = false;
            }
        }
        ...
        if (gIsDragging) {
            ...
            if (gDraggedBlockIndex == -1) {
                if (inPlay) {
                    ...
                    int idx = max(0, min((int) gExecuteList.size(), relY / 80));
                    assignOpResultName(gDraggedBlock);
                    gExecuteList.insert(gExecuteList.begin() + idx, gDraggedBlock);
                }
            } else {
                if (inMenu) { gExecuteList.erase(gExecuteList.begin() + gDraggedBlockIndex); } else if (inPlay) {
                    int relY = gMouse.y - 190 + gMouse.scrolloffsetBlocksPlayGround;
                    int ni = max(0, min((int) gExecuteList.size() - 1, relY / 80)), oi = gDraggedBlockIndex;
                    if (ni != oi) {
                        Block mv = gExecuteList[oi];
                        gExecuteList.erase(gExecuteList.begin() + oi);
                        if (ni > oi)ni--;
                        gExecuteList.insert(gExecuteList.begin() + ni, mv);
                    }
                }
                gDraggedBlockIndex = -1;
            }
        ...
        for (auto &b: gExecuteList) {
            ...
            bool isStrOp1 = (b.type == OP_STR_LEN || (b.type == OP_STR_CHAR && b.isInput1Active) || b.type ==
                             OP_STR_JOIN);
            bool isText = (b.type == LOOKS_SAY || b.type == LOOKS_SAY_SECS || b.type == LOOKS_THINK ||
                           b.type == VAR_SET || b.type == VAR_CHANGE || b.type == MYBLOCK_DEFINE || b.type ==
                           MYBLOCK_CALL || isStrOp1);
            if (isCond) {
                bool ok = isalnum(c) || c == ' ' || c == '>' || c == '<' || c == '=' || c == '!' || c == '.' || c == '-'
                          || c == '_' || c == '(' || c == ')';
        ...
        if (gSelectedMode == "Sounds") {
            ...
            if (e.key.keysym.sym == SDLK_RETURN || e.key.keysym.sym == SDLK_KP_ENTER) {
                for (int i = 0; i < 3; i++) {
                    try {
                        int v = stoi(gSndInput.volStr[i]);
                        gSoundSettings[i].volume = max(0, min(128, v));
                    } catch (...) {
                    }
                    try {
                        int p = stoi(gSndInput.pitchStr[i]);
                        gSoundSettings[i].pitch = max(50, min(200, p));
                    } catch (...) {
                    }
                }
                gSndInput.activeVolInput = -1;
        ...
        for (auto &b: gExecuteList) {
            ...
            if (e.key.keysym.sym == SDLK_BACKSPACE && !act->empty()) act->pop_back();
                ...
                b.isInput1Active = b.isInput2Active = false;
                SDL_StopTextInput();
            }
            break;
        }
        ...
        if (e.key.repeat == 0) {
            if (e.key.keysym.sym == SDLK_DELETE && gSelectedMode == "Code" && !gExecuteList.empty()) gExecuteList.
                    pop_back();
...
bool init() {
    ...
    if (SDL_CreateWindowAndRenderer(1920, 1080, SDL_WINDOW_SHOWN | SDL_WINDOW_MAXIMIZED, &m_window, &m_renderer) < 0)
        return false;
...
int main(int argc, char *args[]) {
    ...
    vector<Button> modes = {
        {
            {0, 80, 150, 50}, "Code", gTheme["bgSecondary"], changeBrightness(gTheme["bgSecondary"], 110),
            gTheme["white"], gTheme["textSecondary"], gTheme["primary"]
        },
        {
            {0, 80, 150, 50}, "Costumes", gTheme["bgSecondary"], changeBrightness(gTheme["bgSecondary"], 110),
            gTheme["white"], gTheme["textSecondary"], gTheme["primary"]
        },
        {
            {0, 80, 150, 50}, "Sounds", gTheme["bgSecondary"], changeBrightness(gTheme["bgSecondary"], 110),
            gTheme["white"], gTheme["textSecondary"], gTheme["primary"]
        },
    };
    bool isMouseDown = false;
    while (gIsRunning) {
        if (gIsExecuting && !gIsPaused) {
            ...
            if (!wSay && !wThink) {
                if (gExecuteIndex < (int) gExecuteList.size()) {
                    ...
                    if (cur.type != START && gExecuteIndex == 0) {
                        gIsExecuting = false;
                        gExecuteIndex = 0;
                    } else {
                        executeBlock(cur.type, gExecuteIndex);
                        gExecuteIndex++;
                    }
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEWHEEL && gSelectedMode == "Code") {
                if (isInRect({110, 190, 300, 840}, gMouse.x, gMouse.y)) {
                    gMouse.scrolloffsetCodeMenu -= event.wheel.y * 20;
                    gMouse.scrolloffsetCodeMenu = max(0, gMouse.scrolloffsetCodeMenu);
                }
                if (isInRect({400, 190, 900, 840}, gMouse.x, gMouse.y)) {
                    gMouse.scrolloffsetBlocksPlayGround -= event.wheel.y * 20;
                    gMouse.scrolloffsetBlocksPlayGround = max(0, gMouse.scrolloffsetBlocksPlayGround);
                }
            }
        }
        SDL_GetMouseState(&gMouse.x, &gMouse.y);
        gMouse.isMouseDown = isMouseDown;
        SDL_Color bg = gTheme["secondary"];
        SDL_SetRenderDrawColor(m_renderer, bg.r, bg.g, bg.b, bg.a);
        SDL_RenderClear(m_renderer);
        renderMenu();
        renderModeSelector(modes);
        if (gSelectedMode == "Code") { renderCodePanel(); } else if (
            gSelectedMode == "Sounds") { renderSoundsPanel(); } else {
            drawRectBorder(110, 190, 1190, 840, gTheme["bgWhite"], gTheme["bgSecondary"], 2);
        }
        renderRunButtons();
        renderGamePreview();
        ...
        if (gSaveNotifyTick > 0 && SDL_GetTicks() - gSaveNotifyTick < 2000) {
            ...
            SDL_Rect toast = {10, 75, 200, 34};
            SDL_RenderFillRect(m_renderer, &toast);
