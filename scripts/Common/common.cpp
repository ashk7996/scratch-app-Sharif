...
void updateScrolloffset(int viewportHeight, int contentHeight, SDL_Event &event) {
    scratchApp.mouse.scrolloffset -= event.wheel.y * 20; // scroll speed

    if (scratchApp.mouse.scrolloffset < 0)
        scratchApp.mouse.scrolloffset = 0;

    if (scratchApp.mouse.scrolloffset > contentHeight - viewportHeight)
        scratchApp.mouse.scrolloffset = contentHeight - viewportHeight;
}
